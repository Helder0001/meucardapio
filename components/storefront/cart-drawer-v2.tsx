...
export function CartDrawer({ open, onClose, tenant, tableInfo }: CartDrawerProps) {
  ...
  // Busca CEP (BrasilAPI primeiro, fallback ViaCEP) e identifica zona de entrega
  const handleCepLookup = async (rawCep: string) => {
    ...
    let logradouro = '', bairro = '', localidade = ''
    let found = false
    ...
    if (!found) {
      setCepError('CEP não encontrado ou sem conexão. Você pode digitar o endereço manualmente.')
      setCepZone(null)
      return
    }
