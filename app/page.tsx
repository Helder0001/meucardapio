...
// Inspirada em cardapioweb.com — mais interativa, moderna, responsiva
...
import {
  ...
  QrCode, CreditCard, Package, TrendingUp, ShieldCheck,
} from 'lucide-react'
...
const features = [
  { icon: '📱', title: 'Cardápio Digital', desc: 'PWA instalável com QR Code por mesa. Atualize preços em tempo real, sem reimprimir nada.', color: 'from-orange-500 to-amber-400', bg: 'bg-orange-50 dark:bg-orange-950/20' },
  { icon: '🛵', title: 'Delivery Completo', desc: 'Zonas por bairro, taxa automática e frete grátis acima de valor. Tudo configurável.', color: 'from-blue-500 to-cyan-400', bg: 'bg-blue-50 dark:bg-blue-950/20' },
  { icon: '🍽️', title: 'Sistema de Mesas', desc: 'QR Code exclusivo por mesa. O cliente pede direto pelo celular, sem esperar garçom.', color: 'from-violet-500 to-purple-400', bg: 'bg-violet-50 dark:bg-violet-950/20' },
  { icon: '⚡', title: 'Pagamento PIX', desc: 'QR Code automático com confirmação instantânea via webhook. Zero taxa manual.', color: 'from-emerald-500 to-green-400', bg: 'bg-emerald-50 dark:bg-emerald-950/20' },
  { icon: '📊', title: 'Kanban em Tempo Real', desc: 'Arraste pedidos entre colunas. Atualização instantânea para toda a equipe.', color: 'from-pink-500 to-rose-400', bg: 'bg-pink-50 dark:bg-pink-950/20' },
  { icon: '💬', title: 'WhatsApp Integrado', desc: 'Notificações automáticas de status do pedido. O cliente sempre sabe onde está o pedido.', color: 'from-green-500 to-emerald-400', bg: 'bg-green-50 dark:bg-green-950/20' },
  { icon: '🖨️', title: 'Impressão Automática', desc: 'Envio automático para impressoras térmicas ao confirmar pedido. Sem digitação na cozinha.', color: 'from-slate-500 to-gray-400', bg: 'bg-slate-50 dark:bg-slate-950/20' },
  { icon: '🤖', title: 'IA nos Produtos', desc: 'Gere descrições irresistíveis com OpenAI ou Gemini. Mais clique = mais pedido.', color: 'from-fuchsia-500 to-pink-400', bg: 'bg-fuchsia-50 dark:bg-fuchsia-950/20' },
]
...
const plans = [
  ...
  { name: 'Pro', price: 99, tagline: 'O favorito dos restaurantes', highlight: true, features: ['Produtos ilimitados', 'Até 5 usuários', 'Delivery completo', 'Cupons e fidelidade', 'WhatsApp integrado', 'Multi-PDV (3 unidades)', 'Suporte prioritário'], cta: 'Assinar Pro' },
...
const stats = [
  { value: '12 mil', label: 'restaurantes ativos' },
  { value: '4 M', label: 'pedidos processados' },
  { value: 'R$ 380 M', label: 'em vendas geradas' },
  { value: '98%', label: 'de satisfação' },
]

const testimonials = [
  { name: 'Patrick Bartholazi', role: 'B3X Burger', text: '100% dos meus pedidos chegam pelo FoodSaaS. É tão simples que não faz sentido voltar ao bloquinho de papel.', revenue: '+R$ 18k/mês' },
  { name: 'Juliana Martins', role: 'Pizzaria da Ju', text: 'O kanban em tempo real mudou nossa cozinha. Nunca mais perdemos pedido por falta de comunicação entre salão e cozinha.', revenue: '+R$ 12k/mês' },
  { name: 'Bruno Felipe', role: 'Oxen Burger', text: 'A integração com PIX e WhatsApp economizou 2 atendentes. O cliente paga, a cozinha recebe, o entregador é notificado — automático.', revenue: '+R$ 22k/mês' },
]

const faqs = [
  { q: 'Preciso de site ou app para usar?', a: 'Não. O FoodSaaS cria seu cardápio digital como PWA — os clientes acessam por link ou QR Code, sem baixar nada.' },
  { q: 'Consigo migrar produtos de outro sistema?', a: 'Sim. Oferecemos importação por planilha Excel ou integração direta com iFood e outros sistemas populares.' },
      ...
      <nav className="sticky top-0 z-50 bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-gradient-to-br from-orange-500 to-amber-400 rounded-xl flex items-center justify-center shadow-sm">
              <span className="text-white font-black text-sm">F</span>
            </div>
            <span className="font-black text-gray-900 dark:text-white text-lg tracking-tight">
              Food<span className="text-orange-500">SaaS</span>
            </span>
          </div>
              ...
              <div className="flex items-center gap-2 px-5 py-3.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/80 dark:bg-gray-800/50">
                ...
                <div className="flex-1 flex justify-center">
                  <div className="bg-gray-200 dark:bg-gray-700 rounded-lg px-4 py-1 text-xs text-gray-500 dark:text-gray-400 font-mono">
                    foodsaas.app/menu/meu-restaurante
                  </div>
                </div>
              </div>
      ...
      <section className="py-24 max-w-5xl mx-auto px-5">
        <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 dark:from-gray-800 dark:to-gray-900 p-12 sm:p-16 text-center border border-gray-700">
          ...
          <div className="relative">
            ...
            <p className="mt-5 text-xs text-gray-500">+12 mil restaurantes já usam · Cancele quando quiser</p>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-gray-100 dark:border-gray-800 py-12">
        <div className="max-w-6xl mx-auto px-5">
          <div className="flex flex-col md:flex-row justify-between items-start gap-10">
            <div className="max-w-xs">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <span className="font-black text-gray-900 dark:text-white text-lg tracking-tight">Food<span className="text-orange-500">SaaS</span></span>
              </div>
          ...
          <div className="mt-10 pt-6 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs text-gray-400">
            <p>© 2025 FoodSaaS. Todos os direitos reservados.</p>
