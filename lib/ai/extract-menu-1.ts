...
async function extractWithGroq(base64: string, mimeType: string): Promise<ImportedMenu | null> {
  ...
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      response_format: { type: 'json_schema', json_schema: importedMenuJsonSchema },
      // CORREÇÃO: max_tokens: 4096 estourava o limite de 1000 tokens de
      // saída por minuto (OTPM) do tier "on_demand" gratuito da Groq pro
      // qwen/qwen3.6-27b — a própria API recusava a requisição inteira
      // com 429 antes mesmo de gerar qualquer token, derrubando o import
      // de cardápio sempre que esse fosse o único provedor configurado.
      // 1000 é o teto reportado pela Groq; cardápios com muitos itens
      // podem sair truncados aqui (é só o 2º de 4 provedores em cascata —
      // configurar XAI_API_KEY, OPENAI_API_KEY ou ANTHROPIC_API_KEY dá
      // mais margem pra cardápios grandes).
      max_tokens: 1000,
    }),
    signal: AbortSignal.timeout(60000),
  })
